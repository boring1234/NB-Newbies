...
def get_action(obs,epsilon,last_attack_tick,current_tick):
    """
    ...
    return_value = 0 # 目前是0，debug用
    
    if current_tick - last_attack_tick < ATTACK_COOL_DOWN:
        print ("Cannot attack") # 基于不能attack做出选择
    else:
        print ("Can attack") # 基于可以attack做出选择
    
    if return_value == 2: # 如果此次action选择attack1，则更新最晚的attack时间
        last_attack = current_tick
    
    return return_value, last_attack # 这是我改的。 这可以然玩家直接操控 agent    
    # return action_idx
...
def get_observation(world_state, life):
    ...
    while world_state.is_mission_running:
        ...
        if world_state.number_of_observations_since_last_state > 0:
            ...
            current_time = observations['TotalTime']

            # Rotate observation with orientation of agent

            ##去掉rotation相关的矩阵旋转：agent的行为是一维的，不需要旋转
    ...
    while global_step < MAX_GLOBAL_STEPS:
        ...
        while not world_state.has_mission_begun:
            ...
            for error in world_state.errors:
                print("\nError:",error.text)
        obs, life, current_time = get_observation(world_state, life)  ##这里pass进去life变量，同时return了life变量来修改life
        last_attack_tick = 0
        ...
        while world_state.is_mission_running:
            ...
            action_idx, last_attack = get_action(obs, epsilon, last_attack_tick, current_time)
            last_attack_tick = last_attack
            ...
            next_obs, life, current_time = get_observation(world_state, life)  #这里同样加了life parameter 和 life 的return
